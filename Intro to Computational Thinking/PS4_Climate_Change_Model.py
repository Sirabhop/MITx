...
temp = data.get_yearly_temp(CITIES[0], 1961)
date = np.array([i for i in range(len(temp))])

model = np.polyfit(date, temp, 2)
...
def r_squared(y, estimated):
    ...
    """
    a = np.sum(np.power(np.subtract(y, estimated), 2))
    b = np.sum(np.power(np.subtract(y, np.mean(y)), 2))
    return 1 - a/b
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    y_est = np.array([0 for i in y])
    for model in models:
        y_est = predict(x, model)
...
round(r_squared(y, x*models[0][0] + models[0][1]), 3)
